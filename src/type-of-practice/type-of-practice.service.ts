...
@Injectable()
export class TypeOfPracticeService {
    constructor(
        @InjectRepository(TypeOfPracticeEntity)
        private readonly zoneRepository: Repository<TypeOfPracticeEntity>
      ) { }

    async createEntity(payload: CreateTypeOfPracticeDto, user: AccountEntity) : Promise<string> {

        const isTypeOfPracticeExist = await this.zoneRepository.findOne({where: {name: payload.name}});
        ...
        try {
             await this.zoneRepository.save(newTypeOfPractice);
    ...
    async findById(id: string): Promise<TypeOfPracticeRO> {
        ...
        const typeOfPractice = await this.zoneRepository.findOne(id);
        if(typeOfPractice) {
    ...
    async delete(id: string): Promise<DeleteResult> {
        ...
        const typeOfPractice = await this.zoneRepository.findOne(id);
        if(typeOfPractice) {
            return await this.zoneRepository.delete(typeOfPractice.id);
        }
